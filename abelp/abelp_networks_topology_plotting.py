...
def main(cfg: DictConfig) -> None:
    ...
    dim_2_sample = int(
        np.where(np.all(params_arr == (dim_2, b, xi, chi, rho_en, k, n, p, en_min, en_max), axis=1))[0][0])
    dim_3_sample = int(
        np.where(np.all(params_arr == (dim_3, b, xi, chi, rho_en, k, n, p, en_min, en_max), axis=1))[0][0])
