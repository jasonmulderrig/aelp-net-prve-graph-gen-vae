...
def main(cfg: DictConfig) -> None:
    ...
    if start_epoch == 0:
        ...
        edges_eval_ap = np.zeros(cfg.train.n_epochs)
        edges_eval_gini = np.zeros(cfg.train.n_epochs)
        edge_type_eval_ap = np.zeros(cfg.train.n_epochs)
        edge_type_eval_gini = np.zeros(cfg.train.n_epochs)
        kl_div = np.zeros(cfg.train.n_epochs)
    else:
        total_eval_compnts = np.loadtxt(total_eval_compnts_filename)
        edges_eval_ap = np.loadtxt(edges_eval_ap_filename)
        edges_eval_gini = np.loadtxt(edges_eval_gini_filename)
        edge_type_eval_ap = np.loadtxt(edge_type_eval_ap_filename)
        edge_type_eval_gini = np.loadtxt(edge_type_eval_gini_filename)
        kl_div = np.loadtxt(kl_div_filename)
        total_eval_compnts[start_epoch] *= 0
        edges_eval_ap[start_epoch] *= 0
        edges_eval_gini[start_epoch] *= 0
        edge_type_eval_ap[start_epoch] *= 0
        edge_type_eval_gini[start_epoch] *= 0
    ...
    for epoch in range(start_epoch, cfg.train.n_epochs):
        ...
        with torch.no_grad():
            ...
            for indx, data in enumerate(val_dataloader):
                ...
                edges_eval_ap[epoch] += edges_eval_ap_metric
                edges_eval_gini[epoch] += edges_eval_gini_metric
                edge_type_eval_ap[epoch] += edge_type_eval_ap_metric
                edge_type_eval_gini[epoch] += edge_type_eval_gini_metric
        ...
        edges_eval_ap[epoch] /= n_val_batches
        edges_eval_gini[epoch] /= n_val_batches
        edge_type_eval_ap[epoch] /= n_val_batches
        edge_type_eval_gini[epoch] /= n_val_batches
        kl_div[epoch] /= n_val_batches

        # Evaluate early stopping on total validation metric with a
        # minimum delta threshold
        total_eval_metric = total_eval_compnts[epoch, 0]
        if total_eval_metric < ((1-cfg.train.min_delta_prop)*min_total_eval_metric):
        ...
        np.savetxt(edges_eval_ap_filename, edges_eval_ap)
        np.savetxt(edges_eval_gini_filename, edges_eval_gini)
        np.savetxt(edge_type_eval_ap_filename, edge_type_eval_ap)
        np.savetxt(edge_type_eval_gini_filename, edge_type_eval_gini)
