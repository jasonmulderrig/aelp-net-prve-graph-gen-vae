...
def main(cfg: DictConfig) -> None:
    ...
    ##### artificial polydisperse end-linked polymer network parameter sample
    ...
    apelp_network_additional_node_seeding_args = (
        [
            ...
            for (sample, dim, b, _, _, _, n, _, _, config) in apelp_network_additional_node_seeding_params_list
        ]
    )
    ...
    apelp_network_additional_nodes_type_params_list = (
        apelp_network_additional_node_seeding_params_list.copy()
    )
    apelp_network_additional_nodes_type_args = (
        [
            (cfg.label.network, cfg.label.date, cfg.label.batch, int(sample), int(n), int(config))
            for (sample, _, _, _, _, _, n, _, _, config) in apelp_network_additional_nodes_type_params_list
        ]
    )
