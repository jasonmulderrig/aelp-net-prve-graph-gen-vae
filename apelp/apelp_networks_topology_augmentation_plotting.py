...
def main(cfg: DictConfig) -> None:
    ...
    for dim in cfg.topology.dim:
        ...
        for coord in range(dim):
            ...
            plt.title(
                "{}D apelp network random node placement".format(dim),
                fontsize=16)
            plt.grid(True, alpha=0.25, zorder=0)
            plt.tight_layout()
            plt_filename += "_{}d_apelp_network_node_label_localization.png".format(dim)
        ...
        for coord in range(dim):
            ...
            plt.title(
                "{}D apelp network Hilbert node labeling".format(dim),
                fontsize=16)
            plt.grid(True, alpha=0.25, zorder=0)
            plt.tight_layout()
            plt_filename += "_{}d_apelp_network_node_label_localization.png".format(dim)
...
if __name__ == "__main__":
    ...
    print(f"Artificial polydisperse end-linked polymer network topology augmentation plotting took {execution_time} seconds to run")