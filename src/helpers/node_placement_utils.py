...
def additional_random_node_placement(
    ...
    Args:
        L_filename (str): Filename for simulation box side lengths.
...
def additional_node_seeding(
        ...
        n: int,
        max_try: int) -> None:
    """Additional node placement procedure for irregular spatial
    networks.
...
def additional_nodes_type(core_nodes_type_filename: str, n: int) -> None:
    """Additional nodes type for irregular spatial networks.

    This function appropriately amends the nodes type array to account
    for additional nodes in the simulation box. This function ought to
    be run after additional nodes have been seeded in the simulation
    box, i.e., after the additional_node_seeding() function.

    Args:
        core_nodes_type_filename (str): Filename for core nodes type.
        n (int): Intended total number of nodes.
    ...
    # array
    n_core_nodes_type = np.shape(core_nodes_type)[0]

    # Pass if the number of nodes presently in the core nodes type array
    # is greater than or equal to the intended number of nodes
    if n_core_nodes_type > n:
        print_str = (
            "The number of nodes presently in the core nodes type "
            + "array is greater than the intended number of nodes. "
            + "You may want to modify the intended number of nodes "
            + "accordingly."
        )
        print(print_str)
    elif n_core_nodes_type == n: pass
    else:
        # Calculate number of nodes to add to the core nodes type array
        n_addtnl = n - n_core_nodes_type

        # Add nodes of type 1 to the core nodes type array
        core_nodes_type = np.concatenate(
            (core_nodes_type, np.ones(n_addtnl, dtype=int)), dtype=int)
        
        # Save the updated core nodes type array
        np.savetxt(core_nodes_type_filename, core_nodes_type, fmt="%d")
...
def hilbert_node_label_assignment(
    ...
    Args:
        ...
        L (np.ndarray): Simulation box side lengths.
        dim (int): Physical dimensionality of the network; either 2 or 3 (for two-dimensional or three-dimensional networks).
        hilbert_order (int): Order of the Hilbert space-filling curve. Default (and ideal) value is 6.
    
    Returns:
        tuple[np.ndarray, np.ndarray]: Indices of the nodes that sort
        and unsort the node coordinates along the Hilbert space-filling
        curve.
    ...
    # Normalize coordinates by the simulation box side lengths
    ...
    # Determine the indices that sort the Hilbert curve distances.
    # Equivalently, this array returns the original node corresponding
    # to the resulting Hilbert curve sorted node, i.e.,
    # hilbert_to_orig_nodes[hilbert_node] = orig_node.
    hilbert_to_orig_nodes = np.argsort(hilbert_indcs)

    # Determine the indices that unsort the Hilbert curve distances.
    # Equivalently, this array returns the resulting Hilbert curve
    # sorted node corresponding to the original node, i.e.,
    # orig_to_hilbert_nodes[orig_node] = hilbert_node.
    orig_to_hilbert_nodes = np.empty_like(hilbert_to_orig_nodes)
    orig_to_hilbert_nodes[hilbert_to_orig_nodes] = np.arange(
        np.shape(hilbert_indcs)[0], dtype=int)

    return hilbert_to_orig_nodes, orig_to_hilbert_nodes