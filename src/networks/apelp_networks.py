...
from src.helpers.polymer_network_chain_statistics_utils import (
    p_nu_flory_func,
...
def apelp_network_topology_initialization(
    ...
    # pb_to_core_nodes np.ndarray
    pb_to_core_nodes = pb_to_core_nodes[core_pb_nodes].copy()

    # Construct the core_to_pb_nodes list
    core_to_pb_nodes = core_to_pb_nodes_func(core_nodes, pb_to_core_nodes)
    ...
    while True:
        ...
        for chn_end in np.nditer(chn_ends):
            ...
            if rng.random() > xi: continue
                ...
                # Otherwise, the chain has previously been instantiated
                    ...
                    if nghbr_nodes_num == 0: continue
                    else:
                        # Check if there is only one neighbor in the
                        # sampling neighborhood
                        if nghbr_nodes_num == 1: nghbr_node = nghbr_nodes[0]
                        else:
                        ...
                        core_node = int(pb_to_core_nodes[nghbr_node])
        ...
        for chn in range(m):
            ...
            # Chain is a periodic edge
                ...
                conn_edges.append(
                    (int(edges[chn, 0]), int(pb_to_core_nodes[int(edges[chn, 1])])))
                conn_edges_type.append(2)
            elif (n <= edges[chn, 0] < np.inf) and (edges[chn, 1] < n):
                conn_edges.append(
                    (int(pb_to_core_nodes[int(edges[chn, 0])]), int(edges[chn, 1])))
                conn_edges_type.append(2)
    ...
    del core_pb_nodes, pb_to_core_nodes, core_to_pb_nodes
    del core_pb_coords
    ...
    if dnglng_n > 0:
        ...
        core_nodes_type = np.concatenate(
            (core_nodes_type, 2*np.ones(dnglng_n, dtype=int)), dtype=int)
...
def apelp_network_topology(
    ...
    apelp_network_topology_initialization(
        network, date, batch, sample, scheme, dim, b, xi, chi, k, n, en, en_max,
        config, max_try)