...
from src.helpers.graph_utils import lexsorted_edges
from src.helpers.node_placement_utils import (
    additional_node_seeding,
    additional_nodes_type,
...
def core_node_update_func(
        ...
        core_to_pb_nodes: list[np.ndarray],
    ...
    Args:
        ...
        core_to_pb_nodes (list[np.ndarray]): List of np.ndarrays corresponding to the periodic boundary nodes associated with each core node.
    ...
    if core_nodes_anti_k[core_node_updt] == 0:
        ...
        # core node
        pb_nodes_updt = core_to_pb_nodes[core_node_updt]
...
def dangling_chains_update_func(
    ...
    Args:
        ...
        conn_dnglng_edges_type (list[int]): Dangling chain edges type list, where core dangling chains are of type 1, and periodic boundary dangling chains are of type 2.
    ...
    while num_try < max_try:
        ...
        # Free end is a periodic boundary node
            ...
            conn_dnglng_edges_type.append(2)
...
def aelp_network_additional_node_seeding(
        ...
        config: int,
        max_try: int) -> None:
    """Additional node placement procedure for artificial end-linked
    polymer networks.
    ...
    if network not in ["auelp", "abelp", "apelp"]:
        error_str = (
            ...
            + "placement procedure is only applicable for artificial "
    ...
    coords_filename = (
        aelp_filename_str(network, date, batch, sample, config) + ".coords"
    )
...
def aelp_network_additional_nodes_type(
        network: str,
        date: str,
        batch: str,
        sample: int,
        n: int,
        config: int) -> None:
    """Additional nodes type procedure for artificial end-linked polymer
    networks.

    This function generates necessary filenames and calls upon a
    corresponding helper function to amend the nodes type array to
    account for additional nodes in the simulation box.
    ...
    """
    # The artificial end-linked polymer network additional nodes
    # type procedure is only applicable for artificial end-linked
    # polymer networks. Exit if a different type of network is passed.
    if network not in ["auelp", "abelp", "apelp"]:
        error_str = (
            "Artificial end-linked polymer network additional nodes "
            + "type procedure is only applicable for artificial "
            + "end-linked polymer networks. This procedure will only "
            + "proceed if network = ``auelp'', network = ``abelp'', "
            + "or network = ``apelp''."
        )
    ...
    core_nodes_type_filename = (
        aelp_filename_str(network, date, batch, sample, config)
        + "-core_nodes_type" + ".dat"
    )

    # Call additional nodes type helper function
    additional_nodes_type(core_nodes_type_filename, n)
...
def aelp_network_hilbert_node_label_assignment(
    ...
    curve. The node coordinates, network edges, and chain segment
    particle numbers are updated accordingly. The original node
    coordinates, network edges, and chain segment particle numbers are
    copied and saved before the node label assignment procedure takes
    place.
    ...
    if network not in ["auelp", "abelp", "apelp"]:
        error_str = (
            ...
            + "assignment procedure is only applicable for artificial "
            + "end-linked polymer networks. This procedure will only "
            + "proceed if network = ``auelp'', network = ``abelp'', "
            + "or network = ``apelp''."
        )
    ...
    # Gather node indices that sort and unsort the node labels via the
    # Hilbert space-filling curve method
    hilbert_to_orig_nodes, orig_to_hilbert_nodes = (
        hilbert_node_label_assignment(coords, L, dim)
    )
    
    # Reassign node labels and accordingly update edges via the Hilbert
    ...
    # Save updated node types, edges, edge types, and edge contour
    # lengths
